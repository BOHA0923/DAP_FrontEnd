 ...
 * 頂部固定滑動選單佈局
 * 使用範例：
 * <dw-layout-top-drawer></dw-layout-top-drawer>
 * <dw-layout-top-drawer [dwMenuPlacement]="'left'"></dw-layout-top-drawer>
 *
 * 參數：
 * [dwMenuPlacement]: string // Menu滑動的方向：'top' | 'right' | 'bottom' | 'left'
...
@Component({
  selector: 'dw-layout-top-drawer',
  templateUrl: './layout-top-drawer.component.html',
  styleUrls: ['./layout-top-drawer.component.less'],
  // tslint:disable-next-line:use-view-encapsulation
  encapsulation: ViewEncapsulation.None // 樣式不封裝
})
export class DwLayoutTopDrawerComponent implements OnInit, OnDestroy {
  dwLayoutType: string;
  layoutClass = [];
  headerClass = [];
  // dwWidth = 200;
  ...
  // @Input()
  // set breakpoint(breakpoint: DwBreakPoint) { // 觸發響應式佈局的斷點：'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'xxl'
  ...
  @Input()
  set dwMenuPlacement(dwMenuPlacement: string) { // Menu滑動的方向
    this.menuPlacement = dwMenuPlacement;
  }
   ...
   * 點選Menu後，關閉Menu
   */
  public afterClickMenu(menuItem: any): void {
