...
import { CalendarView, CalendarVisibleDateRange, DwPlanCalendarModalService, DwPlanCalendarService } from '@webdpt/framework/plan-calendar';

/**
 * 行事曆預設滿版組件
 */
@Component({
  selector: 'dw-plan-calendar-default',
  templateUrl: './plan-calendar-default.component.html',
  styleUrls: ['./plan-calendar-default.component.less'],
  providers: []
})
export class DwPlanCalendarDefaultComponent implements OnInit, OnDestroy, AfterViewInit {

  @Input() planCalendarTitleKey = 'dw-plan-calendar-title-my'; // 行事曆標題多語言Key，預設：我的行事曆
  @Input() planCalendarCategory = 'APP'; // 行事曆層級：DAP（平台級），TENANT（企業級），APP（應用級），USER（用戶級）
