 ...
 */
@Component({
  selector: 'dw-plan-calendar-event-add',
  templateUrl: './plan-calendar-event-add.component.html',
  styleUrls: ['./plan-calendar-event-add.component.less'],
  providers: [
    { provide: DwComponent, useExisting: forwardRef(() => DwPlanCalendarEventAddComponent) }
  ]
})

export class DwPlanCalendarEventAddComponent extends DwComponent implements OnChanges {
  /**
   * @see 新增和修改不同
   */
  eventDate: Date;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.dwVisible.currentValue && this.eventDate && !isNaN(this.eventDate.getDate())) {
      this.dataInit();
    }
  }
  ...
  public dataInit(): void {
    ...
    if (this.title === undefined) {
      this.title = this.translateService.instant('dw-plan-calendar-add-schedule');
    }

    this.dwPlanCalendarManagerService.getBelongCalenars().subscribe(
      (belongCalenars: Array<any>) => {
        this.calendarList = belongCalenars;
      }
    );

    if (!isNaN(this.eventDate.getTime())) {
      const initEventDate = this.dwPlanCalendarService.allDayTime(true, this.eventDate, this.eventDate);
      const startDatetime = initEventDate.startDatetime;
      const endDatetime = initEventDate.endDatetime;

      this.eventData.eventInfo.startDatetime = startDatetime;
      this.eventData.eventInfo.endDatetime = endDatetime;

      this.eventData.calAttendeePermission = 'MANAGE';
  ...
  save(): void {
    ...
    if (saveDataObserverCount === 0) {
       ...
       * @see 新增和修改不同
       */
      this.dwPlanCalendarEventManageService.add(params).subscribe( // see End ---------------
        (response: any) => {
          this.handleOk();
        },
        (errorResponse: any) => {
