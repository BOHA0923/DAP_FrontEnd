  ...
  var Slider =
    ...
    function () {
      function Slider(container, _ref) {
        ...
        this.onRotateMove = _ref.onRotateMove || noop
      }
    ...
    function () {
      function ImageViewer(element) {
        ...
        _defineProperty(this, "_resizeSnapHandle", function (imgWidth, imgHeight, imgLeft, imgTop) {
          ...
          _this._state.originSnapHandleDim = Object.assign({}, _this._state.snapHandleDim);
        });
        ...
        value: function _initImageSlider() {
          ...
          var imageSlider = new Slider(imageWrap, {
            ...
            onStart: function onStart(e, position) {
              // console.log('onStart');
              rotateInfo = _this2._getRotateInfo();
              var image = _this2._elements.image; // 圖片element
              var imageTop = parseFloat(css(image, "top"));
              var imageLeft = parseFloat(css(image, "left"));
              imageStartPosition = {
                top: imageTop,
                left: imageLeft
              }
            ...
            onRotateMove: function onRotateMove(e, position) {
              var imageCurrentDim = _this2._getImageCurrentDim(); // 圖片目前長寬
              var containerDim = _this2._state.containerDim; // container 長寬
              var image = _this2._elements.image; // 圖片element
              var imageTop = imageStartPosition.top; // onStart開始時圖片的位置
              var imageLeft = imageStartPosition.left; // onStart開始時圖片的位置
              var canMoveX = 0;
              var canMoveY = 0;
              var originImageTop = 0;
              var originImageLeft = 0;
              var hasMovedX = 0;
              var hasMovedY = 0;
              var rotatedScaledImage = { w: 0, h: 0 };
              rotatedScaledImage.h = imageCurrentDim.w * rotateInfo.scale;
              rotatedScaledImage.w = imageCurrentDim.h * rotateInfo.scale;
              originImageTop = containerDim.h / 2 - imageCurrentDim.h / 2;
              originImageLeft = containerDim.w / 2 - imageCurrentDim.w / 2;
              canMoveX = Math.abs(containerDim.w / 2 - rotatedScaledImage.w / 2);
              canMoveY = Math.abs(containerDim.h / 2 - rotatedScaledImage.h / 2);

              // console.log('canMoveX:' + canMoveX);
              // console.log('canMoveY.h:' + canMoveY);
              // console.log('originImageTop:'+ originImageTop);
              // console.log('originImageLeft:'+ originImageLeft);

              var newLeft = imageLeft + position.dx;
              var newTop = imageTop + position.dy;
              hasMovedX = newLeft - originImageLeft;
              hasMovedY = newTop - originImageTop;
              // console.log('hasMovedX:'+hasMovedX);
              // console.log('hasMovedY:'+hasMovedY);

              if (hasMovedX > 0 && hasMovedX > canMoveX) { // 往右移
                newLeft = newLeft - (hasMovedX - canMoveX);
              }
              if (hasMovedX < 0 && Math.abs(hasMovedX) > canMoveX) { // 往左移
                newLeft = newLeft + (Math.abs(hasMovedX) - canMoveX);
              }
              if (hasMovedY > 0 && hasMovedY > canMoveY) { // 往下移
                newTop = newTop - (hasMovedY - canMoveY);
              }
              if (hasMovedY < 0 && Math.abs(hasMovedY) > canMoveY) { // 往上移
                newTop = newTop + (Math.abs(hasMovedY) - canMoveY);
              }
            ...
            onMove: function onMove(e, position) {
              if (rotateInfo.isTransAddAngle) {
                _this2._sliders.imageSlider.onRotateMove(e, position);
                return;
              }
              var imageCurrentDim = _this2._getImageCurrentDim(); // 圖片目前長寬
              var containerDim = _this2._state.containerDim; // container 長寬
              ...
              var canMoveX = 0;
              var canMoveY = 0;
              var originImageTop = 0; // 一開始置中時圖片位置
              var originImageLeft = 0; // 一開始置中時圖片位置
              var hasMovedX = 0;
              var hasMovedY = 0;

              originImageTop = containerDim.h / 2 - imageCurrentDim.h / 2;
              originImageLeft = containerDim.w / 2 - imageCurrentDim.w / 2;
              canMoveX = Math.abs(containerDim.w / 2 - imageCurrentDim.w / 2);
              canMoveY = Math.abs(containerDim.h / 2 - imageCurrentDim.h / 2);
      ...
      {
        key: "_getRotateInfo",
        value: function _getRotateInfo() {
          // var _thisRotate = this;
          var rotate = 0;
          var scale = 1;
          var largeImageStyle = this._elements.container.querySelector('.iv-image').style;
          var tr = largeImageStyle.getPropertyValue("-webkit-transform") ||
            largeImageStyle.getPropertyValue("-moz-transform") ||
            largeImageStyle.getPropertyValue("-ms-transform") ||
            largeImageStyle.getPropertyValue("-o-transform") ||
            largeImageStyle.getPropertyValue("transform");
          if (tr) {
            var rotateRegexp = /rotate\((.*)deg\)/;
            var scaleRegexp = /scale\((.*)\)/;
            var ms = tr.match(rotateRegexp);
            var scaleMt = tr.match(scaleRegexp);
            if (ms && ms.length > 1) {
              rotate = parseFloat(ms[1]);
            }
            if (scaleMt && scaleMt.length > 1) {
              scale = parseFloat(scaleMt[1]);
            }
          }
          const isTransAddAngle = (Math.abs(rotate) / 90) % 2 === 1;
          this._state.rotate = rotate;
          this._state.scale = scale;
          this._state.isTransAddAngle = isTransAddAngle;
          return {
            rotate: rotate,
            scale: scale,
            isTransAddAngle: isTransAddAngle
          }
        }
      },
      {
        ...
        value: function _calculateDimensions() {
          ...
          this._state.originImageDim = Object.assign({}, this._state.imageDim);
          ...
          this._state.originSnapImageDim = Object.assign({}, this._state.snapImageDim);
