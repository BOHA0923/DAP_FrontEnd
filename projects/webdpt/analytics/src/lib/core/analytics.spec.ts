...
import {
  DwAnalytics,
  ANGULARTICS2_TOKEN,
} from 'projects/webdpt/analytics/src/lib/core/analytics';
import {
  advance,
  createRoot,
...
describe('ngx-analytics', () => {
  ...
  it('is defined', () => {
    expect(DwAnalytics).toBeDefined();
  });
    ...
    it('should track pages by default',
      inject([DwAnalytics],
        (dwAnalytics: DwAnalytics) => {
          expect(dwAnalytics.settings.pageTracking.autoTrackVirtualPages).toBe(true);
      }),
    );
  });
  ...
  describe('Configuration', function() {
    ...
    it('should configure virtualPageviews',
      inject([DwAnalytics], (dwAnalytics: DwAnalytics) => {
        dwAnalytics.virtualPageviews(false);
        expect(dwAnalytics.settings.pageTracking.autoTrackVirtualPages).toBe(false);
      }),
    );

    it('should configure excluded routes',
      inject([DwAnalytics], (dwAnalytics: DwAnalytics) => {
        dwAnalytics.excludeRoutes(['/abc/def', /\/[0-9]+/]);
        expect(dwAnalytics.settings.pageTracking.excludedRoutes).toEqual(['/abc/def', /\/[0-9]+/]);
      }),
    );

    it('should configure cleaning of ids',
      inject([DwAnalytics], (dwAnalytics: DwAnalytics) => {
        dwAnalytics.clearIds(true);
        expect(dwAnalytics.settings.pageTracking.clearIds).toBe(true);
      }),
    );

    it('should configure developer mode',
      fakeAsync(inject([Router, Location, DwAnalytics],
        (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          // ngx-analytics.settings.developerMode = true;
          dwAnalytics.developerMode(true);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
  ...
  describe('ui-router support', function() {
    ...
    it('should track pages on ui router',
      fakeAsync(inject([StateService, DwAnalytics],
        (state: StateService, dwAnalytics: DwAnalytics) => {
  ...
  describe('router support', function() {
    ...
    it('should track pages on route change',
      fakeAsync(inject([Router, DwAnalytics],
        (router: Router, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
  ...
  describe('excludedRoutes', function() {
    ...
    it('should have empty excludedRoutes by default',
      inject([DwAnalytics], (dwAnalytics: DwAnalytics) => {
        expect(dwAnalytics.settings.pageTracking.excludedRoutes.length).toBe(0);
      }));
    ...
    it('should trigger page track if excludeRoutes is empty',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
          dwAnalytics.settings.pageTracking.excludedRoutes = [];
          (<SpyLocation>location).simulateUrlPop('/abc');
          advance(fixture);
          expect(EventSpy).toHaveBeenCalledWith({ path: '/abc' });
        })));
    ...
    it('should trigger page track if excludeRoutes do not match current route',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
          dwAnalytics.settings.pageTracking.excludedRoutes = ['/def'];
          (<SpyLocation>location).simulateUrlPop('/abc');
          advance(fixture);
          expect(EventSpy).toHaveBeenCalledWith({ path: '/abc' });
        })));
    ...
    it('should not trigger page track if current route is excluded',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
          dwAnalytics.settings.pageTracking.excludedRoutes = ['/abc'];
          (<SpyLocation>location).simulateUrlPop('/abc');
          advance(fixture);
          expect(EventSpy).not.toHaveBeenCalledWith({ path: '/abc' });
        })));
    ...
    it('should not allow for multiple route exclusions to be specified',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          ...
          dwAnalytics.settings.pageTracking.excludedRoutes = ['/def', '/abc'];
          (<SpyLocation>location).simulateUrlPop('/abc');
          advance(fixture);
          expect(EventSpy).not.toHaveBeenCalledWith({ path: '/abc' });
    ...
    it('should allow specifying excluded routes as regular expressions',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
          dwAnalytics.settings.pageTracking.excludedRoutes = [/\/sections\/\d+\/pages\/\d+/];
  ...
  describe('clearIds', function() {
    ...
    it('should not clear ids by default',
      inject([DwAnalytics], (dwAnalytics: DwAnalytics) => {
        expect(dwAnalytics.settings.pageTracking.clearIds).toBe(false);
      }),
    );
    ...
    it('should not change url if clearIds is false',
      fakeAsync(inject([Router, Location, DwAnalytics],
         (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
          fixture = createRootWithRouter(router, RootCmp);
          dwAnalytics.pageTrack.subscribe((x) => EventSpy(x));
          dwAnalytics.settings.pageTracking.clearIds = false;
    ...
    it('should remove ids and uuids from url if clearIds is true',
      ...
      (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
        fixture = createRootWithRouter(router, RootCmp);
        dwAnalytics.pageTrack.subscribe((x: any) => EventSpy(x));
        dwAnalytics.settings.pageTracking.clearIds = true;
    ...
    it('should remove ids using custom regex if idsRegExp is set',
      ...
      (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
        ...
        dwAnalytics.pageTrack.subscribe((x: any) => EventSpy(x));
        dwAnalytics.settings.pageTracking.clearIds = true;
        dwAnalytics.settings.pageTracking.idsRegExp = /^[a-z]\d+$/;
    ...
    it('should remove query params if clearQueryParams is set',
      ...
      (router: Router, location: Location, dwAnalytics: DwAnalytics) => {
        fixture = createRootWithRouter(router, RootCmp);
        dwAnalytics.pageTrack.subscribe((x: any) => EventSpy(x));
        dwAnalytics.settings.pageTracking.clearQueryParams = true;
        (<SpyLocation>location).simulateUrlPop('/0sections0/a01/pages/page/2/summary?param=456');
        advance(fixture);
  ...
  describe('EventEmiters', function() {
    ...
    it('should subscribe to and emit from ' + event,
      fakeAsync(inject([DwAnalytics],
        (dwAnalytics: DwAnalytics) => {
          fixture = createRoot(RootCmp);
          for (const event of EventEmiters) {
            (<any>dwAnalytics)[event].subscribe((x: any) => EventSpy(x));
            (<any>dwAnalytics)[event].next(`test: ${event}`);
            advance(fixture);
